...
def loop_buster_dws(s, t, rel_list, f1, f2, f3, f4):      
    if f1.num_sector[s - 1] == t or (s, t) in rel_list:
        t = f2.to_sector[s - 1]
        if s == t or (s, t) in rel_list:
            t = f3.to_sector[s - 1]
            if s == t or  (s, t) in rel_list:
                t = f4.to_sector[s - 1]
...
def loop_buster_ups(s, t, rel_list, k1, k2, k3, k4):      
    if k1.to_sector[t - 1] == s or (s, t) in rel_list:
        s = k2.num_sector[t - 1]
        if s == t or  (s, t) in rel_list:
            s = k3.num_sector[t - 1]
            if s == t or (s, t) in rel_list:
...
def dws(d, c1, f1, f2, f3, f4):
    res = {}
    db = {}
    ds = {name: pd.DataFrame() for name in d}
    ...
    for j in range(len(c1)):
        ...
        for i in range(len(d)-1):
            s = d[i]
            t = d[i+1]   
            ds[t] = f1.to_sector[ds[s]-1]
            ds[t] = loop_buster_dws(ds[s], ds[t], res[j+1], f1, f2, f3, f4)
            res[j+1].append((ds[s], ds[t]))
            db[j+1].append(ds[t])
                  
    df = pd.DataFrame.from_dict(db,orient='index', columns=['c'] + d)
    return df    


def ups(u, c1, k1, k2, k3, k4):
    res = {}
    db = {}
    ...
    for j in range(len(c1)):
        ...
        for i in range(len(u)-1):
            ...
            us[s] = loop_buster_ups(us[s], us[t], res[j+1], k1, k2, k3, k4)
...
def analisis(selected_ctry, selected_year):
    ...
    df = dws(d, c1, f1, f2, f3, f4)
    df2 = ups(u, c1, k1, k2, k3, k4)
...
with container3:
    st.write(' ')
    st.write(' ')
    st.dataframe(analisis(selected_ctry, selected_year), width = 1500 ,height=850)
